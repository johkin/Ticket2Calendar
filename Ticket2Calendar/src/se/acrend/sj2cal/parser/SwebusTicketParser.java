...
public class SwebusTicketParser extends MessageParserBase implements
		MessageParser {
	...
	public SwebusTicketParser(PreferencesInstance preferencesInstance) {
		super();
		this.preferencesInstance = preferencesInstance;
		format = new SimpleDateFormat("dd MMMM yyyyHH:mm",
	...
	public boolean supports(final String sender, final String message) {
		...
		if (preferencesInstance.isParseSwebus()) {
	...
	public SwebusTicket parse(final String message) {
		...
		String date = findValue(message, "den (\\d{1,2} \\D{4,12} \\d{4})",
				"datum");

		String from = findValue(message, "\\d{1,2}\\:\\d{2} (.*) -",
				"avreseort");
		String fromTime = findValue(message, "(\\d{1,2}\\:\\d{2}) .* -",
				"avgångstid");

		String to = findValue(message,
				"-\\s*\\d{1,2}\\:\\d{2} (.*)\\s*Välkommen", "ankomstort");
		String toTime = findValue(message,
				"-\\s*(\\d{1,2}\\:\\d{2}) .*\\s*Välkommen", "ankomsttid");
		try {
